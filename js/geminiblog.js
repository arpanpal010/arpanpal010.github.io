...
function register(entryUrl, title, pubDate, tags) { //required entryUrl
    ...
    console.log("Loaded entry: "+ entry.index + ": " + entry.title + " " + pd.toLocaleDateString());
...
function getEntryByIndex(eindex) {
    if (entry_list.length === 0){ return false;}
    if (entry_list.length === 1){ return entry_list[0];}
    for (var i = 0; i < entry_list.length; i++) {
        //alert(entry_list[i].id + " " + eid);
        if (entry_list[i].index === eindex) {return entry_list[i];}
    }
    //alert(entry_list[i].id+" "+eid);
    return false;
}
            ...
            if (html) {
                ...
                if (entry.tags.length > 0) {
                    ...
                    for(var j = 0; j <entry.tags.length; j++ ){
                        ...
                        ta.setAttribute("href", "#!tag=" + entry.tags[j]);
                ...
                var sa = document.createElement("a");
                sa.setAttribute("id", entry.id);
                sa.setAttribute("href", "#!post=" + entry.id);
                ...
                sra.setAttribute("href", "#!post=" + entry.id);
...
function detailsView(entry, divName) {
    ...
    if (html) {
        ...
        //markdown src
        if (html) {
            //create the text and link element
            var mdsrc=document.createElement("div");
            mdsrc.setAttribute("class", "markdown-source");
            mdsrc.appendChild(document.createTextNode("( The source markdown file for this entry can be found "));
            var mdsrcLink=document.createElement("a");
            mdsrcLink.setAttribute("href", entry.url)
                mdsrcLink.appendChild(document.createTextNode("here"));
            mdsrc.appendChild(mdsrcLink);
            mdsrc.appendChild(document.createTextNode(" )"));
            //append to wrapper
            wrapper.appendChild(mdsrc);
        }

        //entry previous / next buttons
        //add the div container
        var preNexDiv=document.createElement("div");
        preNexDiv.setAttribute("class", "previous-next-links");
        //link for previous
        if ( entry.index > 0) {
            var preLink=document.createElement("a");
            preLink.setAttribute("id", "previous-link");
            preLink.setAttribute("href", "#!post=" + getEntryByIndex(entry.index - 1).id);
            preLink.setAttribute("title", getEntryByIndex(entry.index - 1).title);
            preLink.appendChild(document.createTextNode("\u2190 Older"));
            preNexDiv.appendChild(preLink);
        }

        //link for next
        if ( entry.index < entry_list.length - 1) {
            var nextLink=document.createElement("a");
            nextLink.setAttribute("id", "next-link");
            nextLink.setAttribute("href", "#!post=" + getEntryByIndex(entry.index + 1).id);
            nextLink.setAttribute("title", getEntryByIndex(entry.index + 1).title);
            nextLink.appendChild(document.createTextNode("Newer \u2192"));
            preNexDiv.appendChild(nextLink);
        }

        wrapper.appendChild(preNexDiv);


        //DISQUS - passing entry.url as disqus identifier, to auto create the reply threads
        //alert(entry.url.substring(1));
        if (enableDisqus && disqus_shortname) {
            //dsqs will be loaded inside this div
            var dsqsDiv=document.createElement("div");
            dsqsDiv.setAttribute("id", "disqus_thread");

            //add the elements
            wrapper.appendChild(dsqsDiv);
            //wrapper.appendChild(dsqsScript);
        }
        //add to container
        container.appendChild(wrapper);
        ...
        resetDisqus(entry.id, document.location, entry.title, "en");
    }
}
...
function showArchive(divName) {
    divName = divName || containerDiv;
    var parentContainer = document.getElementById(divName);
    clearElements(parentContainer);
    var container=parentContainer.appendChild(document.createElement("article"));
...
function showTags(divName) {
    if(tags_list.length > 0) {
        ...
        for(var i = 0; i< tags_list.length; i++) {
            ...
            tp.setAttribute("onclick", "document.location.href = '#!tag=" + tag + "';");

            var ta = document.createElement("a");
            ta.setAttribute("href", "#!tag=" + tag);
...
function contactLinkMaker() { //contactLinks defined in config
    ...
    function resetDisqus(newIdentifier, newUrl, newTitle, newLanguage) {
        ...
        console.log("Updated DISQUS info;");
    }
    ...
    function initialize () {
        ...
        if(anchor !== ""){
            ...
            switch (true) {
                case anchor === "frontpage"    : return snippetView();
                case anchor === "tags"         : return showTags();
                case anchor === "archive"      : return showArchive();

                //parse posts by regex
                case /^post=(.*)/.test(anchor) :
                    if (getEntryById(anchor.match(/^post=(.*)/)[1])) {
                        return detailsView(getEntryById(anchor.match(/^post=(.*)/)[1]));
                    } else {document.location.href = "#!frontpage";}

                //parse tags by regex
                case /^tag=(.*)/.test(anchor)  : 
                    if (anchor.match(/^tag=(.*)/)[1]) {
                        return snippetView(getEntriesByTag(anchor.match(/^tag=(.*)/)[1]));
                    } else {document.location.href = "#!frontpage";}

                default                        : document.location.href = "#!frontpage";
            }
        }
